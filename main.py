...
def calculate_row_column(plaintext: str):
    row = int(plaintext[1:6], 2)
    column = int(plaintext[0] + plaintext[6] + plaintext[7], 2)
    return row, column
...
def keygen(key: str):
    global subkey_list
    for i in range(0, 8):
        split_key = [(key[i:i + 32]) for i in range(0, len(key), 32)]
        subkey_list.append(split_key[0])
        subkey_list.append(split_key[1])
        subkey_list.append(split_key[2])
        subkey_list.append(split_key[3])
        key = key[12:] + key[:12]
...
def algorithm(plaintext: int):
    for i in range(0, 8):
        plaintext = fistelBox(plaintext)

    result = "{0:x}".format(plaintext)
    return "0x" + result


def wBox(plaintext: int):
    plaintext = format(plaintext, '032b')
    splitPlaintext = [(plaintext[i:i + 8]) for i in range(0, len(plaintext), 8)]

    row, column = calculate_row_column(splitPlaintext[0])
    sBox1Result = int(box1[row][column], 16)

    row, column = calculate_row_column(splitPlaintext[1])
    sBox2Result = int(box2[row][column], 16)

    row, column = calculate_row_column(splitPlaintext[2])
    sBox3Result = int(box3[row][column], 16)

    row, column = calculate_row_column(splitPlaintext[3])
    sBox4Result = int(box4[row][column], 16)

    add1 = add(sBox1Result, sBox2Result)

    xor1 = add1 ^ sBox3Result

    finalResult = add(xor1, sBox4Result)

    return finalResult


def normal(plaintext: int, key: int):
    plaintext = format(plaintext, '064b')
    splitPlaintext = [(plaintext[i:i + 32]) for i in range(0, len(plaintext), 32)]
    halfLeft = int(splitPlaintext[0], 2)
    halfRight = int(splitPlaintext[1], 2)

    xor1 = halfLeft ^ key
    wBoxResult = wBox(xor1)

    xor2 = wBoxResult ^ halfRight

    finalResult = int(format(xor2, '032b') + splitPlaintext[0], 2)
    return finalResult


def last(plaintext: int, keys: list):
    plaintext = format(plaintext, '064b')
    splitPlaintext = [(plaintext[i:i + 32]) for i in range(0, len(plaintext), 32)]
    halfLeft = int(splitPlaintext[0], 2)
    halfRight = int(splitPlaintext[1], 2)

    xor1 = halfLeft ^ keys[0]
    xor2 = halfRight ^ keys[1]

    finalResult = int(format(xor2, '032b') + format(xor1, '032b'), 2)
    return finalResult


def fistelBox(plaintext: int):
    for r in range(0, 32):
        subkey = int(subkey_list[r], 2)
        plaintext = normal(plaintext, subkey)

    keys = [int(subkey_list[30], 2), int(subkey_list[31], 2)]
    finalResult = last(plaintext, keys)
    return finalResult


inputText = input("Plain Text : ")
input_key = input("key : ")
# padding
inputText = int(format(int(inputText[2:], 16), '064b'), 2)
input_key = int(format(int(input_key[2:], 16), '0128b'), 2)
keygen(format(input_key, '0128b'))

print(algorithm(plaintext=inputText))
